...
# Connection hardening shared by the login test and every tunnel: long connect
# timeout, in-connection retries, and keepalives to survive flaky links.
ssh_hardening_args=(
  -o "ConnectTimeout=90"
...
# Reverse forwards published on the target server back to services reachable
# from this container. Every mapping shares one dedicated SSH connection, kept
# apart from the failover SOCKS tunnels. Each entry in SECOND_IP_REVERSE_PORTS
# is "REMOTE_PORT" or "REMOTE_PORT:TARGET_HOST:TARGET_PORT", separated by space
# or comma; missing target host/port default to host.docker.internal:REMOTE_PORT.
